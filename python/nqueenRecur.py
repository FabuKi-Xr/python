...
# import sys
# def nQueen(combo,cols,sol):
#     if combo[-1] != combo[0]:
#         if N==len(set(combo[0][i]+i for i in cols))==len(set(combo[0][i]-i for i in cols)):
#             if sol == None : sol = 0
#             sol += 1
#         else:
#             sol = nQueen(combo[:-1],cols,sol)
            
#             #print('Solution '+str(sol)+': '+str(combo[-1])+'\n')
#     return int(sol)  
#             #print("\n".join(' o ' * i + ' X ' + ' o ' * (N-i-1) for i in combo[-1]) + "\n\n\n\n")
# N = int(input("Enter n : "))
# sys.setrecursionlimit(6000)
# print(nQueen(list(permutations(range(N))),range(N),sol=0))
print("========== recursion ============")
for i in range(7):
    N = i+4			 # N x N Board 
